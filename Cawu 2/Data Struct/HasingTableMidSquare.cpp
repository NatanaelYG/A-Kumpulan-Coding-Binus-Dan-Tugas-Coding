...
int hash(const char *key) {
    ...
    // snprintf melakukan write kepada memory buffer (ini ada di stack memory), dalam case ini yg di write adalah variable squared_key
    // %lld merupakan format string, jadi kita ingin si squared_key diperlakukan sebagai long long integer
    // kalau ingat %d itu format string untuk print integer, nah ini ditambah 'll' karena tipe datanya long long integer jadi 'lld'
    // setelah di write ke buffer nanti di store ke variable tujuan, dalam case ini str_key
    ...
    if (len % 2 == 0) {
    	...
    	// strat pos = len/2 artinya kita ada di posisi 2 (digit ke 3) 
